...
function build_curl {
    ...
    build_nghttp2
    local flags="--prefix=$BUILD_PREFIX --with-nghttp2=$BUILD_PREFIX --with-zlib=$BUILD_PREFIX --with-ssl=$BUILD_PREFIX --without-libidn2 --without-libpsl"
...
function build_gdal {
    ...
    if [ -n "$IS_MACOS" ]; then
        rm $BUILD_PREFIX/lib/libpng* || true
    fi
